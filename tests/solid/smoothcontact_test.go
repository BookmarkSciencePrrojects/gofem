...
func test_contact01a(tst *testing.T) {
	...
	// start simulation
	main := fem.NewMain("data/contact01.sim", "", true, false, false, false, chk.Verbose, 0)
	...
	// check Qmap
	e1 := dom.Elems[1].(*solid.Solid)
	chk.Ints(tst, "e1.Qmap", e1.Qmap, []int{20, 23, 28})
	e3 := dom.Elems[3].(*solid.Solid)
...
func Test_contact01b(tst *testing.T) {
	...
	// start simulation
	main := fem.NewMain("data/contact01.sim", "", true, true, false, false, chk.Verbose, 0)
