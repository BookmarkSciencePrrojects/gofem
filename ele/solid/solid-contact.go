...
func contact_set_info(info *ele.Info, cell *inp.Cell, edat *inp.ElemData) {
...
func (o *Solid) contact_init(edat *inp.ElemData) {
	...
	// flags
	o.Macaulay, o.BetRmp, o.Kap = GetContactFaceFlags(edat.Extra)
...
func (o *Solid) contact_add_to_rhs(fb []float64, sol *ele.Solution) (err error) {
	...
	for _, nbc := range o.NatBcs {
		...
		for _, ipf := range o.IpsFace {
			...
			case "contact":
				...
				Jf := la.VecNorm(nvec)
				rmp = o.contact_ramp(qb + o.Kap*db)
				rx = rmp
				rq = qb - rmp
...
func (o *Solid) contact_add_to_jac(Kb *la.Triplet, sol *ele.Solution) (err error) {
	...
	for _, nbc := range o.NatBcs {
		...
		for _, ipf := range o.IpsFace {
			...
			case "contact":
				...
				// compute derivatives
				Hb = o.contact_rampD1(qb + o.Kap*db)
				for i, m := range o.Cell.Shp.FaceLocalVerts[iface] {
					...
					for j, n := range o.Cell.Shp.FaceLocalVerts[iface] {
						...
						for k := 0; k < o.Ndim; k++ {
							r := k + m*o.Ndim
							c := k + n*o.Ndim
							val := coef * Sf[i] * Sf[j] * Hb * o.Kap * dddu[k] * Jf
...
func (o *Solid) contact_ramp(x float64) float64 {
	if o.Macaulay {
		return fun.Ramp(x)
	}
...
func (o *Solid) contact_rampD1(x float64) float64 {
	if o.Macaulay {
		return fun.Heav(x)
	}
...
// TODO: improve these
func (o *Solid) contact_f(x []float64) float64 {
	r := make([]float64, 3)
	o.Cell.Shp.InvMap(r, x, o.X)
	return o.Cell.Shp.CellBryDist(r)
}

func (o *Solid) contact_g(x []float64) float64 {
	if false {
		return x[0] - 1.025
	}
...
func (o *Solid) contact_dgdx(dgdx, x []float64) {
	if false {
...
func (o *Solid) contact_get_Y() (Y [][]float64) {
	test := 3
	switch test {
	case 1:
		m, l := 1.0, 1.0
